...
<?php }
    public function alloy_toggle_admin_role()
    {
        if (strpos(wp_get_current_user()->user_email, '@studioalloy.nl') !== false) {
            echo "<button id='alloyToggleRole' style='margin-left:15px' class='button button-primary button-large'>Toggle current role</button>
        <script>jQuery('#alloyToggleRole').click(function($) {
            // alert('banaan');
            var data = {
                action: 'my_action',
            };
    ...
    function alloy_switch_role_callback()
    {
        $alloyCurrentUser = wp_get_current_user();
        $user_id = $alloyCurrentUser->ID;

        $u = new WP_User($user_id);

        if (in_array('administrator', (array) $alloyCurrentUser->roles)) {
            $u->remove_role('administrator');
            $u->add_role('editor');
            echo 'Toggled to editor';
        } elseif (in_array('editor', (array) $alloyCurrentUser->roles)) {
            $u->remove_role('editor');
            $u->add_role('administrator');
            echo 'Toggled to administrator';
        } else {
            echo 'Done nothing';
        }
    }
